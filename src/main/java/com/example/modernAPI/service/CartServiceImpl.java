...
import com.example.modernAPI.repository.CartRepository;
...
public class CartServiceImpl implements CartService {
  ...
  public List<Item> addOrReplaceItemsByCustomerId(String customerId, @Valid Item item) {
    ...
    List<ItemEntity> items =
        Objects.nonNull(entity.getItems()) ? entity.getItems() : List.of(); // Objects.nonNull()은 객체가 null이 아닌지 확인하는 메서드다. 객체가 null이 아닐 때 true를 반환하고, null이면 false를 반환한다.
    AtomicBoolean itemExists = new AtomicBoolean(false); // 일반적인 boolean 변수는 람다식(forEach) 내부에서 값을 변경할 수 없음. AtomicBoolean을 사용하면 람다식 내부에서도 안전하게 값 변경 가능.
    items.forEach(i -> {
    ...
    return itemService.toModelList(repository.save(entity).getItems()); // 저장된 최신 장바구니 아이템 목록을 API 응답으로 반환한다.
  }
